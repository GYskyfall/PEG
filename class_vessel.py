...
class VESSEL:
  ...
  def AscentSimulation(self, v_start, pitch_init, target_altitude, target_vz, stage):
    ...
    t = self.t 
    t_noPEGupdate = 6     #time before burnout [s] where PEG parameters are not calculated anymore
    ...
    while t < (self.t + self.s2_tb):   #burnout of stage 2
      ...
      #log PEG parameters
      PEGparameters = np.append(PEGparameters,[[t, self.PEG_A, self.PEG_B, self.PEG_C, self.PEG_T]],axis=0)
